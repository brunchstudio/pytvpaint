...
#
#
# IS_NOT_TVP12 = not tv_version()[1].startswith('12')
#
#
...
# @pytest.mark.parametrize("name", LAYER_NAMES_TO_TEST)
# def test_tv_layer_folder_create(test_project: TVPProject, name: str) -> None:
#     new_layer = tv_layer_create(name, layer_type=0)
#     assert tv_layer_info(new_layer).type == LayerType.FOLDER
#     assert tv_layer_info(new_layer).name == name
#
#
...
# def test_tv_layer_folder_delete(test_project: TVPProject, remove_children) -> None:
#     new_layer = tv_layer_create("destroy", layer_type=0)
#     tv_layer_folder_delete(new_layer, remove_children=remove_children)
#
#     # Layer shouldn't exist anymore
#     with pytest.raises(NoObjectWithIdError):
#         tv_layer_get_pos(new_layer)
