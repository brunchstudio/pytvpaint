...
def tv_load_sequence(
    ...
    Returns the number of images of the new layer
    """
...
def tv_bookmarks_enum(position: int) -> int:
    """
    Get the image index (in the clip) corresponding
    to the bookmark at the given position
    """
    return int(
...
def tv_clip_save_structure_json(
    ...
    dict_args = {
        ...
        "excludenames": exclude_layer_names,
    }
...
def tv_clip_save_structure_psd(
    ...
    image: image will only export the given image
    mark_in, mark_out: markin will export a psd sequence between a start and end frame
    """
    export_path = Path(export_path)
...
def tv_sound_clip_reload(clip_id: int, track_index: int) -> None:
    ...
    Also you can give any track_index you want and it returns 0
    TODO: it doesn't accept a proper clip id and it works with a sound clip id...
    """
