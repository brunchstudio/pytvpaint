...
class Project(Refreshable):
    ...
    def position(self) -> int:
        for pos, project_id in enumerate(self.opened_project_ids()):
    ...
    def exists(self) -> bool:
        ...
        return self.path.is_file() and self.path.suffix == ".tvpp"
    ...
    def get_project(
        ...
        for project in Project.opened_projects():
            if (by_id and project.id == by_id) or (by_name and project.name == by_name):
                return project
        raise ValueError("Can't find a project")
    ...
    def clip_names(self) -> Iterator[str]:
        """Optimized way to get the clip names"""
        for scene_id in self.current_scene_ids():
    ...
    def get_clip(
        ...
        if scene_id is not None:
            scene = self.get_scene(by_id=scene_id)

        search_list = scene.clips if scene else self.clips
        return get_tvp_element(search_list, by_id, by_name)
    ...
    def mark_in(self, value: int | None) -> None:
        """set value to None to clear/delete"""
        if value:
